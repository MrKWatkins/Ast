...
public sealed class SourceFilePositionTests
{
    ...
    public void Constructor_ThrowsIfStartIndexIsLessThanZero()
    {
        ...
        AssertThat.Invoking(() => new TestPosition(file, -1, 5))
            .Should().Throw<ArgumentOutOfRangeException>()
            .That.Should().HaveMessage($"Value must be greater than 0. (Parameter 'startIndex'){Environment.NewLine}Actual value was -1.");
    }
    ...
    {
        var file = new TestFile("Test Name", 10);
        AssertThat.Invoking(() => new TestPosition(file, 0, -1))
            .Should().Throw<ArgumentOutOfRangeException>()
    ...
    public void Constructor_ThrowsIfStartIndexIsGreaterThanOrEqualToTheFileLength(int fileLength, int startIndex)
    {
        ...
        AssertThat.Invoking(() => new TestPosition(file, startIndex, 1))
            .Should().Throw<ArgumentOutOfRangeException>()
            .That.Should().HaveMessage($"Value must be less than file's length. ({fileLength}) (Parameter 'startIndex'){Environment.NewLine}Actual value was {startIndex}.");
    }
    ...
    {
        var file = new TestFile("Test Name", 10);
        AssertThat.Invoking(() => new TestPosition(file, 8, 3))
            .Should().Throw<ArgumentOutOfRangeException>()
            .That.Should().HaveMessage($"Value plus startIndex (8) must be less than file's length. (10) (Parameter 'length'){Environment.NewLine}Actual value was 3.");
    }
    ...
    public void Constructor(int fileLength, int startIndex, int length, int expectedEndIndex)
    {
        ...
        position.StartIndex.Should().Equal(startIndex);
        position.Length.Should().Equal(length);
        position.EndIndex.Should().Equal(expectedEndIndex);
    }
    ...
    public void Overlaps(int startIndexX, int lengthX, int startIndexY, int lengthY, bool expected)
    {
        ...
        positionX.Overlaps(positionY).Should().Equal(expected);
        positionY.Overlaps(positionX).Should().Equal(expected);
    }
    ...
    public void ToString_Test()
    {
        ...
        position.ToString().Should().Equal("Test Name (3, 2)");
    }
    ...
    public void Combine_DifferentFiles()
    {
        ...
        positionX.Invoking(x => x.Combine(positionY))
            ...
            .That.Should().HaveMessage("Value is for a different file. (Parameter 'other')");
    ...
    public void Addition()
    {
        ...
        (positionX + positionY).Should().BeTheSameInstanceAs(combined);
    }
